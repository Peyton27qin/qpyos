// Physical memory allocator, for user processes,
// kernel stacks, page-table pages,
// and pipe buffers. Allocates whole 4096-byte pages.

#include "types.h"
#include "param.h"
#include "memlayout.h"
#include "spinlock.h"
#include "riscv.h"
#include "defs.h"

#define PA2PGREF_ID(p) (((p) - KERNBASE) / PGSIZE)   	//由物理地址获取物理页id
#define PGREF_MAX_ENTRIES PA2PGREF_ID(PHYSTOP)			//物理页上限
int pageref[PGREF_MAX_ENTRIES];							//每个物理页的引用数；
struct spinlock pgreflock;								//用于pageref数组的锁

#define PA2PGREF(p) pageref[PA2PGREF_ID((uint64)(p))] 	//获取地址对应物理页引用数

void freerange(void *pa_start, void *pa_end);

extern char end[]; // first address after kernel.
                   // defined by kernel.ld.

struct run {
  struct run *next;
};

struct {
  struct spinlock lock;
  struct run *freelist;
} kmem;

void
kinit()
{
  initlock(&kmem.lock, "kmem");
  initlock(&pgreflock, "pgref"); 	//初始化锁
  freerange(end, (void*)PHYSTOP);
}

void
freerange(void *pa_start, void *pa_end)
{
  char *p;
  p = (char*)PGROUNDUP((uint64)pa_start);
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    kfree(p);
}

// Free the page of physical memory pointed at by v,
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(void *pa)
{
  struct run *r;

  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    panic("kfree");

  //当前页面引用数<= 0的时候释放页面
  acquire(&pgreflock);//上锁避免多个进程同时修改
  if(--PA2PGREF(pa) <= 0){
    // memset就是把物理页里填上1，意思是这个物理页空闲了
    memset(pa, 1, PGSIZE);

    r = (struct run*)pa;
    //把r插入空闲链表指针
    acquire(&kmem.lock);
    r->next = kmem.freelist;//空闲链表头指针
    kmem.freelist = r;
    release(&kmem.lock);
  }

  release(&pgreflock);
}

// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
void *
kalloc(void)
{
  struct run *r;

  acquire(&kmem.lock);
  r = kmem.freelist;
  if(r)
    kmem.freelist = r->next;
  release(&kmem.lock);

  if(r){
    memset((char*)r, 5, PGSIZE); // fill with junk
    PA2PGREF(r) = 1;	//将新分配的物理页的引用数目设置为1
    }
  return (void*)r;
}

//物理引用页数+1
void cyh_krefpage(void* pa){
    acquire(&pgreflock);
    PA2PGREF(pa)++;
    release(&pgreflock);
}

void* cyh_kcopy_n_deref(void* pa){
    acquire(&pgreflock);
    //当前物理页的引用数为1，不需要分配
    if(PA2PGREF(pa) <= 1){
        release(&pgreflock);
        return pa;
    }
    //分配新的物理页，并且将旧页中的数据复制到新页
    uint64 newpa = (uint64)kalloc();
    if (newpa == 0){
        release(&pgreflock);
        return 0;
    }
    //memmove是复制物理页的函数
    memmove((void*) newpa, (void*) pa, PGSIZE);
    
    //旧页的引用数减一
    PA2PGREF(pa)--;
    release(&pgreflock);
    return (void*)newpa;
}
